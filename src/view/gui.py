...
from src.view import scoreTable as st
from src.controller import genAllChart as g, frameobj_helper as hp
from src.helper import query as qr
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 设置窗口图标
        self.setWindowIcon(QIcon('static/icons/star.png'))
        # 设置窗口大小
        ...
        url = os.path.abspath('static/icons/foobar.html')
        # 指定打开界面的 URL
        ...
        if (not os.path.exists(os.path.curdir + '/static/modeHtml')):
            os.mkdir(os.path.curdir + '/static/modeHtml')
        ...
        self.normalUrls = os.listdir(os.path.curdir + '/static/chartHtml/')
        ...
        for action in self.scoreMenu.actions():
            if action.isChecked():
                ...
                if (str == 'top-k-score'):
                    ...
                    # top-k查询结果进行店铺评分
                    title = str
                    k_list = src.controller.no_frameobj_helper.top_k(d_dict, self.k, isReturnList=True)
                    ...
                    # range查询结果进行店铺评分
                    title = str
                    r_list = src.controller.no_frameobj_helper.top_r(d_dict, self.r, isReturnList=True)
    ...
    def showDataChart(self):
        ...
        for action in self.fileMenu.actions():
            if action.isChecked():
                url = os.path.abspath("static/modeHtml/" + action.text() + '.html')
    ...
    def showNormalChart(self):
        for action in self.normalMenu.actions():
            if action.isChecked():
                url = os.path.abspath("static/chartHtml/" + action.text() + '.html')
                # remember to unchecked !
                action.setChecked(False)
                self.browser.load(QUrl.fromLocalFile(url))
                self.setCentralWidget(self.browser)
    ...
    def addChartMenu(self):
        self.urlList = os.listdir(os.path.curdir + '/static/modeHtml')
