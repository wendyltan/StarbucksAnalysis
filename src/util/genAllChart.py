...
class Gen():
    ...
    def __init__(self):
        ...
        # # 第2次迭代，需求1按经纬度绘制散点图
        dc.draw_map(self.starbucks, continent='world',folder='chartHtml/')
        #
        # # 第2次迭代，需求2按时区绘制地图
        hp.set_random_color_for_df(self.starbucks, "Timezone")
        dc.draw_map(self.starbucks, continent='world', isTimeZone=True,folder='chartHtml/')
        #
        # # 第2次迭代，需求2按时区绘制条形图
        timezone = hp.count_number_to_dict(self.starbucks, "Timezone")
        x_timezone = []
        y_number = []
        # # 不排序
        ...
        dc.gen_Bar(x_timezone, y_number, "2.2统计每个时区中星巴克的数量",folder='chartHtml/')
        #
        # # 第2次迭代，需求3按国家中星巴克数量绘制地图
        self.starbucks = hp.count_stabucks_quantity_for_df(self.starbucks)
        self.starbucks = hp.change_alpha2_to_alpha3_for_df(self.starbucks)
        dc.draw_map_by_country(self.starbucks, title="2.3全世界各国星巴克数量",folder='chartHtml/')
        #
        # # 第2次迭代，需求4按国家中星巴克数量绘制条形图
        country = {}
        x_country = []
        y_number = []
        for country_name in self.starbucks["Country"]:
        ...
        for item in sorted(country.items(), key=lambda item: item[1], reverse=True):
            ...
            y_number.append(item[1])
        dc.gen_Bar(x_country, y_number, "2.4统计每个国家拥有的星巴克数量",folder='chartHtml/')
        # # 画世界/亚洲等范围的地图
    ...
    def run(self, lon,lat, r_or_k,keyword=None,mode='k'):
        ...
        if(lon == 0 and lat == 0 and r_or_k == 0 and keyword == None and mode ==''):
            ...
            dc.draw_map(self.starbucks,isTimeZone=True,isOpen=False,scl=scl,export=False,newtitle="3.1 店铺密度")
