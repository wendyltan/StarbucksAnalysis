...
class Gen():
    ...
    def __init__(self):
        ...
        # # 第2次迭代，需求1按经纬度绘制散点图
        dc.draw_map(self.starbucks.df, continent='world',folder='chartHtml/')
        #
        # # 第2次迭代，需求2按时区绘制地图
        self.starbucks = hp.set_random_color_for_df(self.starbucks, "Timezone")
        dc.draw_map(self.starbucks.df, continent='world', isTimeZone=True,folder='chartHtml/')
        #
        # # 第2次迭代，需求2按时区绘制条形图
        ...
        dc.draw_map_by_country(self.starbucks.df, title="2.3全世界各国星巴克数量",folder='chartHtml/')
        #
    ...
    def run(self, lon,lat, r_or_k,keyword=None,mode='k'):
        ...
        if(mode=='k'):
            ...
            # 用户输入：目标点经纬度，k值
            k_list = src.util.no_frameobj_helper.top_k(d_dict, k=r_or_k, isReturnList=True)
            print(k_list)
            ...
            # 用户输入：目标点经纬度，k值
            src.util.no_frameobj_helper.show_query_delay(d_dict, r_or_k, "K", st)
        elif(mode=='r'):
            ...
            st = time.time()
            d_dict = hp.count_all_distance(aimlat, aimlng, self.starbucks)
            r_list = src.util.no_frameobj_helper.top_r(d_dict, r=r_or_k, isReturnList=True)
            hp.show_info_in_map(aimlat, aimlng, self.starbucks, r_list, t='4.2.1')

            # 第4次迭代，需求2.2，展示range查询时延的变化
            # 用户输入：目标经纬度
            src.util.no_frameobj_helper.show_query_delay(d_dict, r_or_k, "R", st)
        elif(mode=='m'):
